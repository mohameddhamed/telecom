# lottery_client_stdin.py
import sys
import socket

# --- Solves Task I.a (TCP) + Task II.a (bytes with ':') + Task IV.b (stdin input) ---


def run_lottery_client(ip, port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        try:
            ...
            print("Enter 5 numbers (1-20), separated by spaces:")
            numbers_input = input().strip().split()
            picked_numbers = [int(n) for n in numbers_input[:5]]

            print("Enter bet amount:")
            bet_amount = int(input().strip())
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python3 lottery_client_stdin.py <server_ip> <server_port>")
        sys.exit(1)

    server_ip = sys.argv[1]
    server_port = int(sys.argv[2])

    run_lottery_client(server_ip, server_port)
